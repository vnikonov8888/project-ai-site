...
function VideoCard({ number, title, description, glow }: VideoItem) {
  const glowShadow =
    glow === "yellow"
  ...
  const placeholderGlow =
    glow === "yellow"
      ? "radial-gradient(ellipse at 50% 30%, rgba(245,216,61,0.18) 0%, rgba(11,11,15,0) 70%)"
      : "radial-gradient(ellipse at 50% 30%, rgba(108,108,245,0.22) 0%, rgba(11,11,15,0) 70%)";

  return (
    <div
      className={`relative overflow-hidden rounded-3xl border border-white/10 bg-black ${glowShadow}`}
    >
      {/* Static cinematic placeholder — no video, no JS */}
      <div className="aspect-[9/16]">
        <div
          className="h-full w-full"
          style={{ background: placeholderGlow }}
          aria-hidden
        >
          <div className="flex h-full w-full items-center justify-center">
            <span
              className="font-serif text-[6rem] font-bold leading-none"
              style={{
                color:
                  glow === "yellow"
                    ? "rgba(245,216,61,0.12)"
                    : "rgba(108,108,245,0.15)",
              }}
            >
              AI
            </span>
